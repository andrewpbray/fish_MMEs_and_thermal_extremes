...
```{r}
lasso_fit_1 <- read_rds("data/models/log_loss_opt.rds")
lasso_fit_2 <- read_rds("data/models/log_loss_opt_5_repeats.rds")
```

```{r}
ggplot(lasso_fit_1)

m <- lasso_fit_1$finalModel
plot(m)
lambdas <- m %>%
  tidy() %>%
  select(lambda) %>%
  distinct() %>%
  pull()
lambda_diff <- abs(lambdas - m$lambdaOpt)
coef(m)[, which.min(lambda_diff)]


ggplot(lasso_fit_2)
m2 <- lasso_fit_2$finalModel
plot(m2)
lambdas2 <- m2 %>%
  tidy() %>%
  select(lambda) %>%
  distinct() %>%
  pull()
lambda_diff2 <- abs(lambdas2 - m2$lambdaOpt)
tibble(coef_1 = coef(m)[, which.min(lambda_diff)],
       coef_2 = coef(m2)[, which.min(lambda_diff)])
```

```{r}
tidy(m) %>%
  mutate(diff = abs(lambda - m$lambdaOpt)) %>%
