...
m <- ridge_f1_logloss$finalModel %>%
  coef(ridge_f1_logloss$bestTune$lambda) 
coef_ridge_f1_logloss <- tibble(term = row.names(m), estimate = m[,1]) %>%
  filter(term != "(Intercept)") %>%
  select(term, estimate) %>%
  add_column(model = "Ridge F1, logloss")
plot_coefs(coef_ridge_f1_logloss)

m <- ridge_f1_logloss_downsampled$finalModel %>%
  coef(ridge_f1_logloss_downsampled$bestTune$lambda) 
coef_ridge_f1_logloss_downsampled <- tibble(term = row.names(m), estimate = m[,1]) %>%
  filter(term != "(Intercept)") %>%
  select(term, estimate) %>%
  add_column(model = "Ridge F1, logloss, downsampled")
plot_coefs(coef_ridge_f1_logloss_downsampled)

m <- ridge_f2_logloss_downsampled$finalModel %>%
  coef(ridge_f2_logloss_downsampled$bestTune$lambda) 
coef_ridge_f2_logloss_downsampled <- tibble(term = row.names(m), estimate = m[,1]) %>%
  filter(term != "(Intercept)") %>%
  select(term, estimate) %>%
  add_column(model = "Ridge F2, logloss, downsampled")
plot_coefs(coef_ridge_f2_logloss_downsampled)
...
bind_rows(coef_lasso_f1_logloss,
          ...
          coef_ridge_f1_logloss,
          coef_ridge_f1_logloss_downsampled,
          coef_ridge_f2_logloss,
          coef_ridge_f2_logloss_downsampled,
          coef_re_f1) %>%
