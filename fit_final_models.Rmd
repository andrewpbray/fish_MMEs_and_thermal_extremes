...
predictive_lasso <- train(summerkill ~ variance_after_ice_30 + 
                          ...
                          trControl = control_logloss_ds)
stopCluster(cl)
# Adjust intercept to account for downsampling
...
ind_opt <- which.min(abs(predictive_lasso$finalModel$lambda -
                           predictive_lasso$finalModel$lambdaOpt))
predictive_lasso$finalModel$a0[ind_opt] <- predictive_lasso$finalModel$a0[ind_opt] - 
  log(((1 - prev)/prev)*(1))
write_rds(predictive_lasso, "../models/predictive_lasso.rds")
```
...
a$a0[31]
a$lambda[31]
sum(predict(a, future, type = "prob")$pos)

b <- a
prev <- mean(historical_data$summerkill == "pos")
b$finalModel$a0["s30"] <- a$finalModel$a0["s30"] - 
  log(((1 - prev)/prev)*(1))
sum(predict(b, future, type = "prob")$pos)
```
